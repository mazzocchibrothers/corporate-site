...
import { ArrowLeft, ArrowRight, Clock, BookOpen, Users, Target, MessageCircle, Lock, Handshake, FileCheck, HelpCircle } from 'lucide-react';
...
export default async function BlogArticle3() {
  const locale = await getLocale();
  const t = await getTranslations('blog.negotiation-techniques');
  ...
  return (
    ...
    b: (chunks) => <strong className="text-[#121212]/80">{chunks}</strong>,
        ...
        <section className="section-breathe">
          <div className="max-w-[780px] mx-auto px-8 lg:px-12 py-16 lg:py-20">
            <Reveal as="h2" y={20} duration={0.6} className="text-[26px] font-semibold text-[#121212] mb-4 tracking-[-0.02em] leading-[1.15]">
              {t('heading6')}
            </Reveal>
