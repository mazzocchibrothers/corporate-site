...
const CHALLENGE_BUSINESS_CHALLENGES = [
  ...
  { id: 'managingComplexityAt', icon: Users },
  { id: 'needGreaterForesight', icon: BarChart3 },
];

const CHALLENGE_HR_CHALLENGES = [
  { id: 'tooMuchTime', icon: Zap },
  { id: 'difficultyIdentifyingFuture', icon: CheckCircle },
  { id: 'nicheProfilesHard', icon: Target },
];

const OBJECTIVES_ITEMS = [
  { id: 'scalePreScreening', icon: Zap },
  { id: 'makeSoftSkills', icon: Eye },
  { id: 'leverageAiAs', icon: Shield },
  { id: 'selectPotentialNot', icon: Target },
];

const SOLUTION_SKILLS = [
  { id: 'roleSpecificHard', icon: PencilRuler },
  { id: 'softSkills', icon: Brain },
...
export default function EuropAssistanceStoryPage() {
  ...
  const t = useTranslations('customers.europ-assistance');
                ...
                <div className="rounded-2xl border border-white/[0.08] bg-white/[0.04] backdrop-blur-sm p-5">
                  <div className="flex items-center gap-3 mb-3 pb-3 border-b border-white/[0.08]">
                    <div className="w-10 h-10 rounded-lg overflow-hidden shrink-0 bg-white flex items-center justify-center">
                      <img loading="lazy" decoding="async" src="/logos/europ-assistance-logo.png" alt="Europ Assistance logo" className="w-full h-full object-contain" />
                    </div>
                    <div>
                      <span className="text-[10px] font-bold text-white/30 tracking-[0.1em] uppercase block mb-0.5">{t('clientCard.label')}</span>
                      <p className="text-[16px] font-bold text-white/90">{t('clientCard.companyName') || 'Europ Assistance'}</p>
                    </div>
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                {RESULTS_PILLARS.map((p) => {
                  ...
                  <div key={t(`results.pillars.${p.id}.label`)} className="rounded-2xl border p-8" style={{ background: '#b7f5d8', borderColor: '#93e0bb' }}>
                    ...
                    <h4 className="text-[32px] stat-value text-[#0b3b28] mb-3 leading-[1.3]">{t.rich(`results.pillars.${p.id}.value`, {
    br: () => <br />,
  })}</h4>
                    <p className="text-[15px] text-[#0b3b28]/60 leading-[1.55]">{t(`results.pillars.${p.id}.label`)}</p>
                  </div>
                  );
                })}
