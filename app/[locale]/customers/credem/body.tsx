...
const CHALLENGE_BUSINESS_CHALLENGES = [
  { id: 'infrastructureCapableHandling', icon: Zap },
  { id: 'candidateExperienceHad', icon: Heart },
  { id: 'identifyingUnexpressedPotential', icon: TrendingUp },
];

const CHALLENGE_HR_CHALLENGES = [
  { id: 'talentPoolNeeded', icon: BarChart3 },
  { id: 'timeHireNeeded', icon: Users },
  { id: 'selectionQualityNeeded', icon: Target },
];

const OBJECTIVES_ITEMS = [
  { id: 'makeFirstStep', icon: Target },
  { id: 'reduceTimeHire', icon: Zap },
  { id: 'guaranteeConsistentHiring', icon: Layers },
  { id: 'improveCandidateExperience', icon: Heart },
];

const SOLUTION_SKILLS = [
  { id: 'resilienza', icon: Target },
  { id: 'intelligenzaEmotiva', icon: Users },
  { id: 'orientamentoAgliObiettivi', icon: Layers },
];
...
const RESULTS_QUALITATIVE = [
  { id: 'moreCandidatesCan', icon: Eye },
  { id: 'radicallyImprovedInterview', icon: CheckCircle },
  { id: 'humanLoopAlways', icon: Layers },
];

export default function CredemStoryPage() {
  const router = useRouter();
  const lang = useLocale();
  const t = useTranslations('customers.credem');
      ...
      <main>
        ...
        <section className="relative pt-[80px] min-h-screen flex items-center">
          <div className="absolute inset-0 overflow-hidden">
            <img src="/logos/credem_customer_story_cover.avif" alt="" className="absolute inset-0 w-full h-full object-cover" style={{ filter: 'blur(8px) brightness(0.25)', transform: 'scale(1.1)' }} />
                ...
                <div className="rounded-2xl border border-white/[0.08] bg-white/[0.04] backdrop-blur-sm p-5">
                  <div className="flex items-center gap-3 mb-3 pb-3 border-b border-white/[0.08]">
                    <div className="w-10 h-10 rounded-lg overflow-hidden shrink-0 bg-white flex items-center justify-center">
                      <img loading="lazy" decoding="async" src="/logos/credem-logo (2).png" alt="Gruppo Credem logo" className="w-full h-full object-contain " />
                    </div>
                    <div>
            ...
            <Section className="mb-24">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
                {RESULTS_METRICS.map((m, i) => {
                  ...
                  return (
                    <div key={t.rich(`results.metrics.${m.id}.label`, {
    b: (chunks) => <strong className="text-[#0b3b28] font-semibold">{chunks}</strong>,
    br: () => <br />,
  })} className="rounded-2xl border p-8" style={{ background: '#b7f5d8', borderColor: '#93e0bb' }}>
                      ...
                      <h4 className="text-[32px] stat-value text-[#0b3b28] mb-3 leading-[1.3]">{t(`results.metrics.${m.id}.value`)}</h4>
                      <p className="text-[15px] text-[#0b3b28]/60 leading-[1.55]">{t.rich(`results.metrics.${m.id}.label`, {
    b: (chunks) => <strong className="text-[#0b3b28] font-semibold">{chunks}</strong>,
    br: () => <br />,
  })}</p>
                      {t.has(`results.metrics.${m.id}.sublabel`) && <p className="text-[13px] text-[#0b3b28]/45 mt-1">{(t.has(`results.metrics.${m.id}.sublabel`) ? t(`results.metrics.${m.id}.sublabel`) : '')}</p>}
                    </div>
                  );
                })}
              </div>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
                {RESULTS_QUALITATIVE.map((q) => (
                  <div key={t(`results.qualitative.${q.id}.title`)} className="rounded-2xl border border-[#e5e7eb] bg-white p-8">
                    ...
                    <p className="text-[15px] text-[#121212]/55 leading-[1.55]">{t.rich(`results.qualitative.${q.id}.text`, {
    b: (chunks) => <strong className="text-[#121212]/90 font-semibold">{chunks}</strong>,
  })}</p>
                  </div>
                ))}
              </div>
