...
const CHALLENGE_BUSINESS_CHALLENGES = [
  { id: 'noVisibilityHigh', icon: Eye },
  { id: 'wideGapBetween', icon: Users },
  { id: 'highTurnoverStore', icon: TrendingUp },
];

const CHALLENGE_HR_CHALLENGES = [
  { id: 'competencyAnalysisNecessary', icon: Zap },
  { id: 'developmentReservedOnly', icon: Layers },
  { id: 'noStructuredFeedback', icon: BarChart3 },
];

const OBJECTIVES_ITEMS = [
  { id: 'mapSkillsEntire', icon: Eye },
  { id: 'optimiseTimeCost', icon: Zap },
  { id: 'promoteInternalCareer', icon: TrendingUp },
  { id: 'workProactivelyTraining', icon: Target },
];

const SOLUTION_SKILLS = [
  { id: 'industrySpecificHard', icon: Wrench },
  { id: 'keySoftSkills', icon: Heart },
];

const RESULTS_QUALITATIVE = [
  { id: 'skillsFinallyVisible', icon: Eye },
  { id: 'hiddenTalentPredictive', icon: Target },
  { id: 'customisedTrainingSkill', icon: CheckCircle },
  { id: 'timeCostOptimisation', icon: Zap },
];

export default function DouglasStoryPage() {
  const router = useRouter();
  const lang = useLocale();
  const t = useTranslations('customers.douglas');
      ...
      <main>
        ...
        <section className="relative pt-[80px] min-h-screen flex items-center">
          <div className="absolute inset-0 overflow-hidden">
            <img src="/logos/douglas-background-explore-stories.avif" alt="" className="absolute inset-0 w-full h-full object-cover" style={{ filter: 'blur(8px) brightness(0.25)', transform: 'scale(1.1)' }} />
                ...
                <div className="rounded-2xl border border-white/[0.08] bg-white/[0.04] backdrop-blur-sm p-5">
                  <div className="flex items-center gap-3 mb-3 pb-3 border-b border-white/[0.08]">
                    <div className="w-10 h-10 rounded-lg overflow-hidden shrink-0 bg-white flex items-center justify-center">
                      <img loading="lazy" decoding="async" src="/logos/douglas-logo.png" alt="Douglas logo" className="w-full h-full object-contain " />
                    </div>
                    <div>
                      <span className="text-[10px] font-bold text-white/30 tracking-[0.1em] uppercase block mb-0.5">{t('clientCard.label')}</span>
                      <p className="text-[16px] font-bold text-white/90">Douglas</p>
                    </div>
                  </div>
    ...
    br: () => <br />,
            ...
            <Section className="mb-24">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
                {[
                  { ...t.raw('results.metrics')[0], icon: CheckCircle },
                  { ...t.raw('results.metrics')[1], icon: Clock },
                  { ...t.raw('results.metrics')[2], icon: Users },
                ].map((m) => (
