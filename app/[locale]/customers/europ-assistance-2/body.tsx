...
const CHALLENGE_BUSINESS_CHALLENGES = [
  { id: 'needManageSeasonal', icon: Users },
  { id: 'noVisibilityGrowth', icon: BarChart3 },
];

const CHALLENGE_HR_CHALLENGES = [
  { id: 'decisiveSoftSkills', icon: Eye },
  { id: 'hrTimeWas', icon: Zap },
];

const OBJECTIVES_ITEMS = [
  { id: 'scalePreScreening', icon: Zap },
  { id: 'makeSoftSkills', icon: Eye },
  { id: 'buildFairDefensible', icon: Shield },
  { id: 'selectPotentialNot', icon: Target },
];

const SOLUTION_SKILLS = [
  { id: 'softSkills', icon: Heart },
  { id: 'englishLanguage', icon: Wrench },
  { id: 'roleSpecificHard', icon: Layers },
];

const RESULTS_QUALITATIVE = [
  { id: 'visibilityPotentialFrom', icon: BarChart3 },
  { id: 'recruitersFreedHigh', icon: Zap },
  { id: 'positiveCandidateExperience', icon: Heart },
];

export default function EuropAssistance2StoryPage() {
  const router = useRouter();
  const lang = useLocale();
  const t = useTranslations('customers.europ-assistance-2');
