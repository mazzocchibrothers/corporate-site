...
const CHALLENGE_BUSINESS_CHALLENGES = [
  ...
  { id: 'costErrorAmplified', icon: TrendingUp },
];

const CHALLENGE_HR_CHALLENGES = [
  { id: 'fairnessAtVolumes', icon: Scale },
  { id: 'filteringBlockingRequirements', icon: Target },
  { id: 'hiringProcessAs', icon: Heart },
];

const OBJECTIVES_ITEMS = [
  { id: 'process1300', icon: Zap },
  { id: 'mapAllKey', icon: Eye },
  { id: 'ensureFairnessGrowth', icon: Scale },
  { id: 'turnHiringInto', icon: TrendingUp },
];

const SOLUTION_SKILLS = [
  { id: 'leadershipPeopleManagement', icon: Users },
  { id: 'englishLanguage', icon: Shield },
  { id: 'opennessInternationalMobility', icon: Target },
];

const RESULTS_QUALITATIVE = [
  { id: 'clearResultsMatter', icon: CheckCircle },
  { id: 'transfersAlreadyUnderway', icon: Users },
  { id: 'employerBrandingRecognised', icon: Heart },
];

export default function EatalyStoryPage() {
  const router = useRouter();
  const lang = useLocale();
  const t = useTranslations('customers.eataly');
  ...
  return (
                ...
                <div className="rounded-2xl border border-white/[0.08] bg-white/[0.04] backdrop-blur-sm p-5">
                  <div className="flex items-center gap-3 mb-3 pb-3 border-b border-white/[0.08]">
                    ...
                    <div>
                      ...
                      <p className="text-[16px] font-bold text-white/90">Eataly</p>
                    </div>
                  </div>
    ...
    br: () => <br />,
              ...
              <p className="text-[16px] text-[#121212]/[0.65] leading-[1.85]">{t('context.summary')}</p>
            </Section>
