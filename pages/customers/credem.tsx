              ...
              <div className="lg:col-span-8">
                <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.3 }}>
                  <h1 className="text-[clamp(2.2rem,4.5vw,3.8rem)] font-bold tracking-[-0.03em] text-white/95 mb-8" style={{ lineHeight: 1.12 }}>
                    {t('Time-to-hire halved.')}<br />{t('Twice the quality.')}
                  </h1>
                  ...
                  <div className="border-l-2 border-[#4B4DF7]/30 pl-6">
                    <p className="text-[17px] text-white/[0.65] italic leading-[1.7]">{t('"We gave more candidates a chance to prove their abilities, making our search for high-potential talent much more effective. All while cutting time-to-hire by 50%."')}</p>
                    <p className="text-[14px] text-white/50 mt-3 font-semibold">Giuliana Barigazzi, <span className="font-normal text-white/35">{t('HR Recruiter, Credem')}</span></p>
                  </div>
                </motion.div>
              </div>
          ...
          <div className="max-w-[1400px] mx-auto px-8 lg:px-12">
            ...
            <Section className="mb-20">
              ...
              <p className="text-[16px] text-[#1A1A2E]/[0.65] leading-[1.8] max-w-3xl">
                {t("One of Italy's most established banking groups. Every year, 30,000 candidatures arrive. mostly young professionals entering the job market. The challenge wasn't volume. It was finding the high-potential profiles hidden inside it.")}
              </p>
            </Section>
            ...
            <Section className="mb-20">
              ...
              <p className="text-[18px] text-[#1A1A2E]/50 leading-[1.6] mb-12 max-w-3xl italic">
                {t("The first screening filter wasn't selective enough. Strong candidates got lost in the volume.")}
              </p>
              ...
              <div className="grid lg:grid-cols-4 gap-4 mb-10">
                {[
                  ...
                  <div key={s.step} className="rounded-xl border border-[#4B4DF7]/[0.08] bg-white/60 p-6 text-center">
                    ...
                    <h4 className="text-[15px] font-bold text-[#1A1A2E] mb-2">{t(s.title)}</h4>
                    <p className="text-[13px] text-[#1A1A2E]/50 leading-[1.6]">{t(s.desc)}</p>
                  </div>
                ))}
              </div>
              <div className="border-l-2 border-[#4B4DF7]/20 pl-6 max-w-3xl">
                <p className="text-[15px] text-[#1A1A2E]/50 italic leading-[1.7]">{t('"For organizations like ours, managing truly significant candidate volumes, integrating technological support like Skillvue into the selection process is fundamental to exponentially increasing results. both qualitatively and quantitatively."')}</p>
                <p className="text-[13px] text-[#1A1A2E]/40 mt-2 font-semibold">Daniele Candiani, <span className="font-normal">{t('TA & EB Manager, Credem')}</span></p>
              </div>
            </Section>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={row.label} className="rounded-xl bg-white/[0.04] p-5">
                      <span className="text-[11px] font-bold text-white/30 tracking-[0.1em] uppercase block mb-3">{t(row.label)}</span>
                      <p className="text-[13px] text-white/35 line-through mb-2">{t(row.before)}</p>
                      <p className="text-[13px] text-white/[0.65]">{t(row.after)}</p>
                    </div>
                  ))}
                </div>
              </div>
              <div className="border-l-2 border-[#4B4DF7]/20 pl-6 max-w-3xl">
                <p className="text-[15px] text-[#1A1A2E]/50 italic leading-[1.7]">{t('"Today, thanks to Skillvue, we include more people in the first phase while conducting a more effective and selective analysis. saving time in subsequent stages."')}</p>
                <p className="text-[13px] text-[#1A1A2E]/40 mt-2 font-semibold">Alessandro Bonsignori, <span className="font-normal">{t('HR Training Manager, Credem')}</span></p>
              </div>
            </Section>
                ...
                <div className="grid lg:grid-cols-[auto_1fr] gap-8 items-start">
                  ...
                  <div>
                    ...
                    <p className="text-[16px] text-[#1A1A2E]/[0.65] leading-[1.8] max-w-2xl">
                      {t('Now that the value has been proven, Credem is extending Skillvue to the full organization. The same level of quality and selectivity will become the standard for all selection processes. bringing consistent, objective competency evaluation to every hire across 19 regions and 600+ branches.')}
                    </p>
                  </div>
                </div>
              </div>
