      ...
      <div className="relative max-w-[1400px] mx-auto px-8 lg:px-12">
        ...
        <motion.div className="mb-12" initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7 }}>
          <h2 className="text-[clamp(1.8rem,3.5vw,3rem)] font-bold leading-[1.05] tracking-[-0.02em] text-[#121212] max-w-4xl">
            ...
            <span className="font-bold gradient-text-on-light">{t('high-stakes talent decisions')}</span>
          </h2>
        </motion.div>
        ...
        <motion.div className="mb-10" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
          <h2 className="text-[clamp(1.8rem,3.5vw,3rem)] font-bold leading-[1.05] tracking-[-0.02em] text-[#121212]">
            {t('Frequently asked')}{' '}
            <span className="font-bold gradient-text-on-light">{t('questions')}</span>
          </h2>
        </motion.div>

        <div className="max-w-3xl space-y-0">
          {faqs.map((faq, i) => (
            <motion.div key={i} className="border-b border-[#121212]/[0.08]" initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.1 + i * 0.06 }}>
              <button onClick={() => setOpenIdx(openIdx === i ? null : i)} data-testid={`faq-${i}`} className="w-full flex items-center justify-between py-6 text-left">
                <span className="text-[16px] font-semibold text-[#121212]/85 pr-8">{t(faq.q)}</span>
                <ChevronDown className={`h-4 w-4 text-[#121212]/30 shrink-0 transition-transform duration-400 ${openIdx === i ? 'rotate-180' : ''}`} />
              </button>
              <AnimatePresence>
                {openIdx === i && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
                    <p className="text-[15px] text-[#121212]/[0.55] leading-[1.75] pb-6">{t(faq.a)}</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </motion.div>
